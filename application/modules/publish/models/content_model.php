...
* 
* @copyright 2017 CICADA CMS.
* @author InaGicher <iangicher@gmail.com>
* @package Hero Cicada 
* 
*/
		...
		if (isset($filters['id']) or isset($filters['type'])) {
			...
			if (isset($result) and $result->num_rows() == 0) {
				...
				if (isset($filters['keyword']) and $content_count > 10) {
					...
					$this->db->where('(MATCH (' . $search_fields . ') AGAINST ("' . mysqli_real_escape_string($link, $filters['keyword']) . '") OR `content`.`content_title` LIKE \'%' . mysql_real_escape_string($filters['keyword']) . '%\')', NULL, FALSE);
					$this->db->select('MATCH (' . $search_fields . ') AGAINST ("' . mysqli_real_escape_string($link, $filters['keyword']) . '") AS `relevance`', FALSE);
					mysqli_close($link);
				}
