...
class Install extends CI_Controller {
	...
	function index() {
		...
		if ($this->input->post('base_url') != '') {
			...
			// validate MySQL info
			$valid_mysql = FALSE;
			$dbi_host = $this->input->post('db_host');
			$dbi_user = $this->input->post('db_user');
			$dbi_pass = $this->input->post('db_pass');
			$dbi_name = $this->input->post('db_name');
									
			if ($dblink = @mysqli_connect($dbi_host, $dbi_user, $dbi_pass, $dbi_name))
			{
		        $valid_mysql = TRUE;
			}
			...
			if (!isset($error_empty_site) and !isset($error_base_url) and !isset($error_mysql)) {
				...
				// update settings
				mysqli_query($dblink, 'UPDATE `settings` SET `setting_value`=\'' . $this->input->post('site_name') . '\' WHERE `setting_name`=\'site_name\' or `setting_name`=\'email_name\'');
				mysqli_query($dblink, 'UPDATE `settings` SET `setting_value`=\'' . $this->input->post('site_email') . '\' WHERE `setting_name`=\'site_email\'');
				mysqli_close($dblink);
	...
	function admin () {
		...
		$gmt_offset = ($this->input->post('gmt_offset')) ? $this->input->post('gmt_offset') : 'UP2';
